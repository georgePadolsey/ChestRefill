...
import uk.co.georgep.chestrefill.utils.exception.PropertiesException;
...
public class Properties {
	...
	/**
	 * A Method that loads every language from the method {@link uk.co.georgep.chestrefill.utils.UtilValues#getAvailableLanguages()}
	 */
			...
			try {
				...
				if (input == null) {
					...
					new PropertiesException(
							...
							"[!] Sorry, unable to find " + filename + " in the plugin "+ Utils.pluginValues.getPluginName()+" [!]"
					).printStackTrace();
					return;
				...
				// Puts the language in the properties hashmap with the Properties file
	 ...
	 * @see uk.co.georgep.chestrefill.utils.Properties#getProperty(String, String)
